    ...
    template <> class BHKMNR2026FormFactors<VacuumToPiPi> : public FormFactors<VacuumToPP>
    {
        private:
            ...
            _P_residue(const unsigned & k) const
            {
                ...
                const complex<double> psi_residue = _psi_r(_M_fp_I1[k](), _G_fp_I1[k]());
            ...
            _Q_residue(const unsigned & k) const
            {
                ...
                const complex<double> psi_residue = _psi_r(_M_fp_I0[k](), _G_fp_I0[k]());

                complex<double> result = 1.0 / (psi_residue - std::conj(psi_residue));
            ...
            inline PDerivatives _P_derivatives(const complex<double> & psi) const
            {
                ...
                complex<double> L = 2.0 / (psi - 1.0) + 1.0 / (psi + 1.0);

                complex<double> L1 = -2.0 / power_of<2>(psi - 1.0) - 1.0 / power_of<2>(psi + 1.0);

                complex<double> L2 = 4.0 / power_of<3>(psi - 1.0) + 2.0 / power_of<3>(psi + 1.0);

                complex<double> L3 = -12.0 / power_of<4>(psi - 1.0) - 6.0 / power_of<4>(psi + 1.0);

                complex<double> L4 = 48.0 / power_of<5>(psi - 1.0) + 24.0 / power_of<5>(psi + 1.0);
            ...
            inline QDerivatives _Q_derivatives(const complex<double> & psi) const
            {
                const std::size_t num_resonances = stoi(_n_resonances_I0.value());
                ...
                complex<double> L1 = 0.0;
                complex<double> L2 = 0.0;
                complex<double> L3 = 0.0;
                complex<double> L4 = 0.0;
                ...
                result.Q5 = Q_val * (power_of<5>(L) + 10.0 * power_of<3>(L) * L1 + 15.0 * L * L1 * L1 + 10.0 * L * L * L2
                                     + 10.0 * L1 * L2 + 5.0 * L * L3 + L4);
