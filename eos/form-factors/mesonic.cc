...
namespace eos
{
    ...
    {
        std::function<double (const double &)> integrand = [this](const double & q2) -> double
        {
            return this->abs2_f_p(q2);
        };

        const double integral = integrate<1, 1>(
            integrand,
            q2_min,
            q2_max,
            cubature::Config().epsrel(1.0e-5)
        );
